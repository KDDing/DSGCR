...
for (r in 1:runs) {
    ...
    for (i in 1:floor(K)) {
      ...
      LLDT <- Lapla_matrix(SDT)
      LLDS <- Lapla_matrix(RDS_v)
      LLDA <- Lapla_matrix(SDA)
        
      I <- diag(drug_num)
      F <- RDS_v%*%solve(I+lamdaA*LLDA+lamdaS*LLDS+lamdaT*LLDT)
...
data_out <- data.frame(lamdaA,lamdaS,lamdaT,AUC)
